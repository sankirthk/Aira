...
struct AppSettings: Codable, Equatable {
    ...
    static func normalizedPillConfigurations(from configurations: [PillWindowConfiguration]) -> [PillWindowConfiguration] {
    ...
    init(from decoder: Decoder) throws {
        ...
        speechSensitivity = try container.decodeIfPresent(SpeechSensitivity.self, forKey: .speechSensitivity) ?? .medium
        autoScrollWPM = try container.decodeIfPresent(Double.self, forKey: .autoScrollWPM) ?? ManualScrollConfiguration.defaultWPM
        appearanceMode = try container.decodeIfPresent(AppearanceMode.self, forKey: .appearanceMode) ?? .system
        managerTypography = try container.decodeIfPresent(ManagerTypography.self, forKey: .managerTypography) ?? .medium
        liveAnswerDisclosureAccepted = try container.decodeIfPresent(Bool.self, forKey: .liveAnswerDisclosureAccepted) ?? false
        ...
        if let decodedConfigurations = try container.decodeIfPresent([PillWindowConfiguration].self, forKey: .pillConfigurations) {
            pillConfigurations = Self.normalizedPillConfigurations(from: decodedConfigurations)
        } else if let legacyMode = try container.decodeIfPresent(PillContentMode.self, forKey: .pillContentMode) {
            pillConfigurations = Self.normalizedPillConfigurations(
                from: Array(repeating: PillWindowConfiguration(contentMode: legacyMode), count: PillWindowConfiguration.maximumSlotCount)
            )
        } else {
            pillConfigurations = PillWindowConfiguration.defaultSlots
        }

        shortcutToggleNotch = try container.decodeIfPresent(String.self, forKey: .shortcutToggleNotch) ?? "⌘⇧N"
        shortcutTogglePill = try container.decodeIfPresent(String.self, forKey: .shortcutTogglePill) ?? "⌘⇧P"
        shortcutToggleVoiceSync = try container.decodeIfPresent(String.self, forKey: .shortcutToggleVoiceSync) ?? "⌘⇧Space"
        shortcutScrollUp = try container.decodeIfPresent(String.self, forKey: .shortcutScrollUp) ?? "↑"
        shortcutScrollDown = try container.decodeIfPresent(String.self, forKey: .shortcutScrollDown) ?? "↓"
        shortcutEndSession = try container.decodeIfPresent(String.self, forKey: .shortcutEndSession) ?? "Escape"
    }
    ...
    func encode(to encoder: Encoder) throws {
        ...
        try container.encode(Self.normalizedPillConfigurations(from: pillConfigurations), forKey: .pillConfigurations)
...
struct PillWindowConfiguration: Codable, Equatable {
    ...
    static let defaultSlots = Array(repeating: PillWindowConfiguration.default, count: maximumSlotCount)
