...
struct PrompterContentView: View {
    ...
    private var shouldRunVoiceMotion: Bool {
        ownsSynchronizedScroll && sessionStarted && voiceSyncEnabled && isVoiceMotionActive && !isHovered
    }
    ...
    var body: some View {
        GeometryReader { geometry in
            ...
            .onChange(of: playheadCoordinator.progress) { _, newProgress in
                guard usesSessionPlayheadForManualScroll || followsSessionPlayheadForManualScroll || sessionStarted && voiceSyncEnabled else { return }
    ...
    private func handleWordIndexUpdate(_ newIndex: Int?) {
        ...
        guard let lineIndex = lineMetrics.firstIndex(where: { $0.wordRange.contains(idx) }) else { return }
    ...
    private func updateLineMetrics(width: CGFloat) {
        ...
        let nsFont = NSFont(name: appearance.fontName, size: appearance.fontSize) ?? .systemFont(ofSize: appearance.fontSize)
    ...
    private func updateManualScrollDriver() {
        ...
        manualScrollDriver.setAutoScrollEnabled(
            sessionStarted &&
            !voiceSyncEnabled &&
            manualAutoScrollEnabled &&
            autoScrollWPM > 0 &&
            !voiceSync.isPausedByUser &&
            ownsSynchronizedScroll
        )
        manualScrollDriver.setAutoScrollSuppressed(isHovered)
    }
}
...
private enum PrompterDisplayText {
    ...
    static func normalizedDisplayBody(from body: String) -> String {
        ...
        let paragraphs = normalizedNewlines
