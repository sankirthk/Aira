...
struct ScriptEditorTextInsertionTests {
    ...
    @Test func cueStylingAppliesDistinctAnnotationColors() throws {
        ...
        let cueForeground = attributed.attribute(.foregroundColor, at: cueRange.location, effectiveRange: nil) as? NSColor
        let cueBackground = attributed.attribute(.backgroundColor, at: cueRange.location, effectiveRange: nil) as? NSColor
        let plainForeground = attributed.attribute(.foregroundColor, at: 0, effectiveRange: nil) as? NSColor
        let plainBackground = attributed.attribute(.backgroundColor, at: 0, effectiveRange: nil) as? NSColor
