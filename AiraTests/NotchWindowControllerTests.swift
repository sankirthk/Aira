...
struct NotchWindowControllerTests {
    ...
    @Test func notchResizeRespectsMinimumAndMaximumSizeLimits() {
        ...
        #expect(shrunk.height >= NotchWindowController.minimumPanelHeight(notchHeight: 34, appearance: appearance))
    }
    ...
    @Test func nonNotchDisplayCollapsesNotchSizeToZero() {
        ...
        #expect(NotchWindowController.notchSize(notchWidth: 120, notchHeight: 34) == CGSize(width: 120, height: 34))
    }
    ...
    @Test func notchCutoutStaysOpenPastPhysicalSideWalls() {
        let rect = CGRect(x: 0, y: 0, width: 400, height: 160)
        let notchWidth: CGFloat = 120
        let notchHeight: CGFloat = 34
        let path = NotchOverlayGeometry.overlayPath(in: rect, notchSize: CGSize(width: notchWidth, height: notchHeight))
        ...
        let sampleY = max(cutoutDepth - 6, 1)

        #expect(sideOverscan > 1)
        #expect(path.contains(CGPoint(x: physicalLeftWall - (sideOverscan * 0.5), y: sampleY), eoFill: false) == false)
        #expect(path.contains(CGPoint(x: physicalRightWall + (sideOverscan * 0.5), y: sampleY), eoFill: false) == false)
    }
}
