...
struct CollectionSidebarLogicTests {
  ...
  @Test func collectionFilterReturnsOnlyScriptsInSelectedCollection() {
    ...
    let scripts = [
      ScriptMeta(
        id: includedID, title: "Included", lastEdited: .now, wordCount: 10, starred: false),
  ...
  @Test func importLogicReturnsSelectedCollectionIDForCollectionFilter() {
    ...
    #expect(
      DocumentLibraryImportLogic.selectedCollectionID(for: .collection(collectionID))
        == collectionID
    )
  }
  ...
  @Test func starredScriptsAreFilteredAndOrderedByLastEditedDescending() {
    ...
    let ordered = DocumentLibraryFilterLogic.starredScripts(from: [
      oldestStarred, unstarred, newestStarred,
    ])
